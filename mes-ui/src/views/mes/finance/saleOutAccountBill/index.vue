  ...
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :inline="true" :model="queryParams" label-width="90px" size="small">
      ...
      <el-form-item label="销售方式" prop="method">
        <el-select v-model="queryParams.method" clearable placeholder="请选择销售方式">
          <el-option
      ...
      <el-form-item label="开票标记" prop="invoiceMark">
        <el-select v-model="queryParams.invoiceMark" clearable placeholder="请选择开票标记">
          <el-option
    ...
    <el-table v-loading="loading" :data="saleOutAccountBillList" :summary-method="getSummaries" height="550px"
              show-summary @selection-change="handleSelectionChange">
      <el-table-column align="center" type="selection" width="55"/>
      <div v-for="(item,index) in entryFieldExtendList">
        <el-table-column :label="item.fieldName" :prop="item.field" :width="item.width" align="center"
    ...
    <!-- 添加或修改销售出库对账单对话框 -->
    <el-dialog :close-on-click-modal="false" :title="title" :visible.sync="open" append-to-body width="1450px">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="出库日期" prop="outTime">
              <el-date-picker v-model="form.outTime"
                              :disabled="true"
                              clearable
                              placeholder="请选择出库日期"
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="物流信息" prop="logisticsInfo">
              <el-input v-model="form.logisticsInfo" :disabled="true" type="textarea"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="备注" prop="remark">
              <el-input v-model="form.remark" :disabled="true" placeholder="请输入备注" type="textarea"/>
            </el-form-item>
          </el-col>
        </el-row>

        <el-divider content-position="center">销售出库对账明细信息</el-divider>
        <el-table ref="saleOutAccountEntry" :data="saleOutAccountEntryList"
                  :row-class-name="rowSaleOutAccountEntryIndex"
                  @selection-change="handleSaleOutAccountEntrySelectionChange">
          <el-table-column align="center" type="selection" width="50"/>
          <!--  动态属性        -->
          <template v-for="(item,index) in formEntryFieldExtendList">
            <el-table-column :label="item.fieldName" :width="item.width+50" align="center" show-overflow-tooltip='true'>
              <template slot-scope="scope">
                <el-form-item :prop="'saleOutAccountEntryList.'+scope.$index+'.'+formPropFormat(item)"
              ...
              <span v-if="item.type==='date'">
                  ...
                  <span v-else>
                <el-input v-if="item.isSystem!='Y'&&scope.row.extendFieldJson!=null"
                          v-model="scope.row.extendFieldJson[item.field]"/>
...
<script>
import {
  addSaleOutAccountBill,
  delSaleOutAccountBill,
  getSaleOutAccountBill,
  listSaleOutAccountBill,
  updateSaleOutAccountBill
} from "@/api/mes/finance/saleOutAccountBill";
