<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="buyOrderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="单号" prop="number"/>
      <el-table-column align="center" label="状态" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.bill_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column align="center" label="采购员" prop="userName"/>
      ...
      <el-table-column align="center" label="审核人" prop="checker"/>
    ...
    <!-- 添加或修改采购订单对话框 -->
    <el-dialog :title="title" :visible.sync="open" append-to-body width="1350px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="采购日期" prop="buyDate">
              <el-date-picker v-model="form.buyDate"
                              clearable
                              placeholder="请选择采购日期"
                              type="date"
                              value-format="yyyy-MM-dd">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button icon="el-icon-plus" size="mini" type="primary" @click="handleAddBuyOrderEntry">添加</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button icon="el-icon-delete" size="mini" type="danger" @click="handleDeleteBuyOrderEntry">删除
            </el-button>
          </el-col>
        </el-row>
        <el-table ref="buyOrderEntry" :data="buyOrderEntryList"
                  :row-class-name="rowBuyOrderEntryIndex" @selection-change="handleBuyOrderEntrySelectionChange">
          ...
          <el-table-column label="要货日期" prop="needDate" width="240">
            <template slot-scope="scope">
              <el-date-picker v-model="scope.row.needDate" clearable placeholder="请选择要货日期" type="date"
                              value-format="yyyy-MM-dd"/>
            </template>
          </el-table-column>
          ...
          <el-table-column v-if="false" label="是否赠品" prop="isGift" width="150">
            <template slot-scope="scope">
              <el-input v-model="scope.row.isGift" placeholder="请输入是否赠品"/>
            </template>
          </el-table-column>
          <el-table-column v-if="false" label="税率" prop="taxRate" width="150">
            <template slot-scope="scope">
              <el-input v-model="scope.row.taxRate" placeholder="请输入税率"/>
            </template>
          </el-table-column>
          <el-table-column v-if="false" label="税额" prop="totalTax" width="150">
            <template slot-scope="scope">
              <el-input v-model="scope.row.totalTax" placeholder="请输入税额"/>
            </template>
          </el-table-column>
          <el-table-column v-if="false" label="金额" prop="totalPrice" width="150">
            <template slot-scope="scope">
...
<script>
import {addBuyOrder, delBuyOrder, getBuyOrder, listBuyOrder, updateBuyOrder} from "@/api/mes/buy/buyOrder";
